...
describe('options testing case',()=>{
   ...
   it("test expect value to None value",()=>{
      let s:null | {} = {}
      const optAuthor = Option(()=>s)
      console.log(optAuthor)
      expect(optAuthor.isSome()).eq(true)
      expect(optAuthor.isNone()).eq(false)
      s = null
      expect(optAuthor.unwrapOr(33)).toBe(33)
      expect(optAuthor.unwrap()).toBe("None")
   })
})
