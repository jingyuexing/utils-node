...
exports.parseSpectrumData = void 0;
const utils_1 = require("@jingyuexing/utils");
const elementLines = {
    'H': [656.3, 486.1, 434.0],
...
function analyzeSpectrum(wavelengths, absorbance, elementLines) {
    const findClosestIndex = (wavelengths, target) => {
    ...
    const matchElements = (wavelengths, absorbance, elementLines) => {
        ...
        for (const element in elementLines) {
            ...
            matchedElements[element] = lineMatches;
        }
...
function parseSpectrumData(data) {
    ...
    for (const line of data) {
        ...
        if ((0, utils_1.isNumber)(wavelength) && (0, utils_1.isNumber)(intensity)) {
            if (!(0, utils_1.isNone)(wavelength) && !(0, utils_1.isNone)(intensity)) {
...
exports.parseSpectrumData = parseSpectrumData;
...
const analyzedElements = analyzeSpectrum(wavelengths, absorbance, elementLines);
// // 打印分析结果
for (const element in analyzedElements) {
    const matches = analyzedElements[element];
    console.log(`${element}:`);
    matches.forEach((match) => {
        console.log(' - Wavelength:', match.wavelength, 'Absorbance:', match.absorbance);
    });
}
parseSpectrumData(["12", "33", "45"]);
